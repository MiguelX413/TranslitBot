def main(do_rich=True):
    ...
    def conditional_track(sequence, text):
        if do_rich:
            return track(sequence=sequence, description=text)
        else:
            print(text)
            return sequence
    ...
    def gendict(
        ...
        for x in subdicts:
            if x.get("type", "dict") == "dict":
                ...
                aliases_dict = {}
                primary_values = set()
                for entry in conditional_track(
                    sequence=data, text="Separating aliases"
                ):
...
if __name__ == "__main__":
    ...
    rich_enable = parser.parse_args().rich
